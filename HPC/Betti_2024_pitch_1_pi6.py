...
def main(end_time, v_w, x0, file_index, seeds, time_step = 0.05, T_s1 = 180):
    ...
    white_noise_ml = np.random.normal(loc=0, scale=np.pi/6, size=31)
    np.random.set_state(state_before)
...
def run_simulations_parallel(n_simulations, params):
    ...
    seeds_array = np.load(f'./seeds_pitch_1_pi6/seeds_{sys.argv[1]}.npy')
...
def save_binaryfile(results):
    ...
    np.savez(f'./results_pitch_1_pi6/results_{sys.argv[1]}_{time}.npz', t=t,  
                                                            state=state, 
