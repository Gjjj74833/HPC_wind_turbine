...
def generate_large_random_phase(iteration, seed, length=31):
    """
    Constructs a new random phase of specified length by sampling from saved KDE distributions.

    Parameters:
        iteration (int): The iteration number to load the KDE files from.
        length (int): The number of phases to construct (should match the number of saved KDE files).
        seed (int, optional): A seed for reproducibility.

    Returns:
        np.ndarray: A new random phase array of length `length`.
    """
    state_before = np.random.get_state()
    np.random.seed(seed)  
    ...
    # Iterate over saved KDE files to sample new values
    for i in range(length):
        kde_file_path = f"{output_dir}/kde_{i}.pkl"
        with open(kde_file_path, "rb") as f:
            kde = pickle.load(f)  # Load the saved KDE
        
        # Sample one value from the KDE
        new_value = kde.resample(1, seed=np.random.default_rng()).flatten()[0]
        new_random_phase.append(new_value)
        
    np.random.set_state(state_before)

    return np.array(new_random_phase)
...
def main(end_time, v_w, x0, file_index, seeds, time_step = 0.05, T_s1 = 180):
    ...
    white_noise_ml = generate_large_random_phase(int(sys.argv[6])-1, seeds[0])
